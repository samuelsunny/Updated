...
if($_SERVER['REQUEST_METHOD'] == "GET")
{
    // When the user clicks on the create account button
        ...
        $query = "select * from containers where capacity = '{$full}'";
        ...
        if($result)
        {
            if($result && mysqli_num_rows($result) > 0)
            {
                ...
                $toship = array();
                for ($x = 0; $x < count($loaded_containers_data); $x++) {

                    $query = "select onShip from loading_orders where containerId = '{$loaded_containers_data[$x][0]}'";
                    $result = mysqli_query($con, $query); 
                    $on_ship_data = mysqli_fetch_all($result);
      ...
      <div class="row mt-4">
          ...
          <table class="table">
            <thead>
                ...
                <th scope="col" class="text-center">Container Name</th>
                </tr>
            </thead>
            <tbody>
            <?php for ($row = 0; $row < count($loaded_containers_data); $row++) {?>
              <tr>
                ...
                <td class="text-center"><?php echo $loaded_containers_data[$row][1] ?></td>
